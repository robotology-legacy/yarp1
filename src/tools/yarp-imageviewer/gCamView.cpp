...
static gint menuImageSize_CB(GtkWidget *widget, gpointer data)
{
	...
	targetWidth = _inputImg.GetWidth();
	targetHeight = _inputImg.GetHeight();
...
static gint menuImageRatio_CB(GtkWidget *widget, gpointer data)
{
	...
	imgWidth = _inputImg.GetWidth();
	imgHeight = _inputImg.GetHeight();
...
bool yarpImage2pixbuf(YARPImageOf<YarpPixelRGB> *sourceImg, GdkPixbuf* destPixbuf)
{
	...
	src_line_size = sourceImg->GetAllocatedLineSize();
	src_data = sourceImg->GetRawBuffer();
