...
YARPOutputPortOf<TrackerMsg> out_pos;
...
// this was a const. not elegant here.
int LIMIT = 0;
...
void ParseCommandLine(int argc, char *argv[])
{
	...
	sprintf (outpos_name, "%s/outpos", my_name); 
}
...
public:
	...
	void DrawRegions (YARPImageOf<YarpPixelMono>& out, int *vxx, int*vyy)
	{
		...
		// plot the max anyway.
		if (maxx != 0 && maxy != 0)
			AddCircleOutline (*img, WHITE, int(maxx*blocksize+blocksize/2), int(maxy*blocksize+blocksize/2), 10);
	...
	int GetLastMaxAveraged (int& mx, int& my)
	{
		...
		mx = tx[whoismax] + blocksize/2;
		my = ty[whoismax] + blocksize/2;
...
void main(int argc, char *argv[])
{
	...
	YARPOpticFlowBM<YarpPixelMono> of(Size,Size,BLOCKSIZE,MAXFLOW,1,BLOCKINC);
	LIMIT = of.L();
	...
	//SinglePosData target_position;
	TrackerMsg target_position;
...
//	OpenLog ();
	...
	while (1)
	{
		...
		for (int i = 3; i <= 8; i++)
		{
			...
			target_position.aj[i] = cur_position.j[i];
		}			
		...
		for (i = 1; i <= 7; i++)
		{
			...
			target_position.hj[i-1] = cur_positionh(i);
		}
		...
		if (regiongrow.StableMax(10)) // && !tracker.IsTracking())
		{
			...
			{
				// start a new tracker.
				target_position.valid = true;
				target_position.img[0] = sx;
				target_position.img[1] = sy;
			}
			else
			{
				target_position.valid = false;
				target_position.img[0] = 0;
				target_position.img[1] = 0;
			}
		}
		else
		{
		...
		out_pos.Content () = target_position;
		out_pos.Write ();

//		WriteLog (vx, vy, ox, oy, velocity);
	}
}
