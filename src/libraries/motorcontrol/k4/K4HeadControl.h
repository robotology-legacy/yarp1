...
// HeadControl.h
//
#ifndef __K4HeadControlh__
#define __K4HeadControlh__
...
protected:
	...
	static int m_neckroll;
	static int m_neckcrane;
...
//public:
	enum { MaxAxes = 10, TILTSTORE = -100000 };
	...
	int NeckRoll() const { return m_neckroll; }
	int NeckCrane() const { return m_neckcrane; }
	...
	virtual int KismetStyleCalibrate(int axis);
        virtual int CalibrateAll();
 
	// all these are in virtual coordinates.
	virtual int Input (void);
	virtual int PositionCommand (double *command, int *statusbits, int uselimit_flag);

	//CHANGE: is velocity command needed??
	...
	//CHANGE: are velocity params needed??
	...
	virtual int SetDefaultVelocity (int vaxis, double velocity);
        virtual int SetDefaultVelocity(double *velocity, int *bits);
	virtual int SetDefaultAcc (double *acc);
	virtual int SetDefaultAcc (int vaxis, double acc);
        virtual int SetDefaultAcc (double *acc, int *bits);
