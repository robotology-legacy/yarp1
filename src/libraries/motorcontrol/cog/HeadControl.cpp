...
YARPSemaphore YARPGenericControl::m_mutex;		// default = 1
...
int		 YARPHeadControl::m_topdiff = 0;
int		 YARPHeadControl::m_bottomdiff = 0;
int		 YARPHeadControl::m_neckpan = 0;
int		 YARPHeadControl::m_necktilt = 0;

int		 YARPHeadControl::m_headtilt = 0;
int		 YARPHeadControl::m_headroll = 0;
...
int YARPHeadControl::Initialize (const char *name, const char *filename)
{
	...
	if (m_instances == 1)
	{
		...
		if (stat < 0)
		{
			printf ("Warning: boot params do not match...\n");
...
void YARPHeadControl::ReadConfig (const char *filename)
{
	...
	EatLine (fp);
	fscanf (fp, "%d ", &m_eyetilt);
	fscanf (fp, "%d ", &m_lefteye);
	fscanf (fp, "%d ", &m_righteye);
	fscanf (fp, "%d ", &m_topdiff);
	fscanf (fp, "%d ", &m_bottomdiff);
	fscanf (fp, "%d ", &m_neckpan);
	fscanf (fp, "%d\n", &m_necktilt);

	EatLine (fp);
	fscanf (fp, "%d ", &m_headtilt);
	fscanf (fp, "%d\n", &m_headroll);
...
int YARPHeadControl::CheckBootParams (void)
{
	...
	for (int i = 0; i < m_njoints; i++)
	{
		// should be || but it's fine anyway here...
...
int YARPHeadControl::DifferentialCalibration (void)
{
	const double CalibrationSpeed = 5000;
	const double CalibrationAcc = 50000;
	...
	int isdx = 0;
    int stat = isd_initialize("/yarp/isd", &isdx);
    if (stat != 0)
	{
	...
	printf("Gyro reads (y,p,r)=(%10f %10f %10f)\n", yaw, pitch, roll);

	// conversion:
	topgoal = - floor( -416.7*(pitch+roll)); // -  52.8 );
	botgoal = - floor(  416.7*(pitch-roll)); //  - 208.5 );
  
	printf("Converted values (top,bot)=(%d %d)\n", topgoal, botgoal);

	short axes[2]; // = { 3, 4 };
	axes[0] = TopDifferential();
	axes[1] = BottomDifferential();
	double positions[2];
	double vel[2];
	double acc[2];
	...
	positions[0] = topgoal;
	positions[1] = botgoal;
	vel[0] = CalibrationSpeed;
	vel[1] = CalibrationSpeed;
	acc[0] = CalibrationAcc;
	acc[1] = CalibrationAcc;

	// add the offset here.
	// Offset might be expressed in virtual coordinates.
	// Convert before.
	double top_offset, bottom_offset;
	ConvertToReal (m_calibrate_offset[HeadTilt()], 
				   m_calibrate_offset[HeadRoll()], 
				   &top_offset, 
				   &bottom_offset);

	positions[0] += top_offset;
	positions[1] += bottom_offset;

	Lock ();

	// now move to that position.
	meid_move_all (meidx, 2, axes, positions, vel, acc);
	//while (!meid_axis_done(meidx, axes[0]) && 
	//	   !meid_axis_done(meidx, axes[1])) 
	//	   delay(20);

	// now we are on zero!
	ZeroAxis (TopDifferential());
	ZeroAxis (BottomDifferential());
	m_winding[TopDifferential()] = 0;
	m_winding[BottomDifferential()] = 0;

	// LATER: put here the limits.
	// 
	m_limits_max[HeadTilt()] = m_pad_max[HeadTilt()];
	m_limits_max[HeadRoll()] = m_pad_max[HeadRoll()];
	m_limits_min[HeadTilt()] = m_pad_min[HeadTilt()];
	m_limits_min[HeadRoll()] = m_pad_min[HeadRoll()];

	m_calibrated[HeadTilt()] = true;
	m_calibrated[HeadRoll()] = true;
...
int YARPHeadControl::Input (void)
{
	...
	for (int i = 0; i < m_njoints; i++)
	{
		...
		// unfiltered (potentially noisy - very noisy).
	...
	// differential.
	ConvertToVirtual (m_position[TopDifferential()], 
					  m_position[BottomDifferential()],
					  &m_position[TopDifferential()],
					  &m_position[BottomDifferential()]);
	ConvertToVirtual (m_velocity[TopDifferential()], 
					  m_velocity[BottomDifferential()],
					  &m_velocity[TopDifferential()],
					  &m_velocity[BottomDifferential()]);
	Unlock ();
...
int YARPHeadControl::CheckAxisPos (int i, double command)
{
	// command is gonna be final position.
	if (command > m_limits_max[i] ||
		command < m_limits_min[i] ||
...
int YARPHeadControl::PositionCommand (double *command, int *flags)
{
	Lock ();
	...
	if (flags == NULL)
	{
		...
		for (int i = 0; i < m_njoints; i++)
		{
			if (CheckAxisPos (i, command[i] / m_etoangles[i]) < 0 || !m_enabled[i])
			{
				m_set_cmd[i] = m_position[i];
			}
			else
				m_set_cmd[i] = command[i] / m_etoangles[i];
		}

		memcpy (m_set_vel, m_default_vel, sizeof(double) * m_njoints);
		memcpy (m_set_acc, m_default_acc, sizeof(double) * m_njoints);

		// convert the command for the differential.
		ConvertToReal (m_set_cmd[HeadTilt()], m_set_cmd[HeadRoll()], &m_set_cmd[TopDifferential()], &m_set_cmd[BottomDifferential()]);
		// vel and acc do not need to be converted.

		short axes[MaxAxes] = { 0, 1, 2, 3, 4, 5, 6, 7 };
		meid_start_move_all (meidx, m_njoints, axes, m_set_cmd, m_set_vel, m_set_acc);
	}
	else
	{
		// build the array of the joints which are supposed to move.
		short axes[MaxAxes] = { 0, 0, 0, 0, 0, 0, 0, 0 };
		int numaxis = 0;
		...
		if (numaxis > 0)
		{
			...
			meid_start_move_all (meidx, numaxis, axes, m_set_cmd, m_set_vel, m_set_acc);
		}
	}
...
// for differential, the limits are in virtual coordinates too.
...
int YARPHeadControl::VerifyLimits (double interval)
{
	for (int i = 1; i <= m_njoints; i++)
		m_pos(i) = m_position[i-1];
	m_gaze.Apply (m_pos);
	float phi = m_gaze.phi_middle;

	for (i = 1; i <= m_njoints; i++)
		m_pos(i) = m_position[i-1] + m_set_cmd[i-1] * interval;
	m_gaze.Apply (m_pos);
	float pred_phi = m_gaze.phi_middle;

	// limit ~=20 degrees up.
	if (phi < -0.34 && (pred_phi - phi) < 0)
	{
		m_set_cmd[HeadTilt()] = 0;
		m_set_cmd[EyeTilt()] = 0;
		m_set_cmd[NeckTilt()] = 0;
		return -1;
	}
...
int YARPHeadControl::VelocityCommand (double *command, double interval)
{
		...
		(vergence > 20000 && delta_v > 0))
	{
		if (vergence < 1000)
		{
			m_set_cmd[LeftEye()] = 1000 - m_position[LeftEye()] + m_position[RightEye()] + m_set_cmd[RightEye()] * interval;
			m_set_cmd[LeftEye()] /= interval;
		}
		else
		{
			m_set_cmd[LeftEye()] = 20000 - m_position[LeftEye()] + m_position[RightEye()] + m_set_cmd[RightEye()] * interval;
			m_set_cmd[LeftEye()] /= interval;
		}

		//printf ("Vergence limit reached, stop eyes : %d\n", vergence);
