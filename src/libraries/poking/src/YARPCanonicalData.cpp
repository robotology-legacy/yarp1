...
YARPCanonicalStats::YARPCanonicalStats (int size, int nbins)
{
	...
	memset (m_units, NULL, sizeof(YARPCanonicalNeuron *) * m_size);
	...
	for (i = 0; i < m_size; i++) m_mirror_actions[i].Resize (m_nbins, m_nbins);
	for (i = 0; i < ACTIONS; i++) m_mirror_tmp[i].Resize (m_nbins, m_nbins);
