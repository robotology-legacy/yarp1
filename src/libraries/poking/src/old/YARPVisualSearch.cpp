...
#define CYL_BUCKETS 5
#define CYL2_BUCKETS 5
#define LUM_BUCKETS 2
#define VAR_BUCKETS 3
  ...
  void AddHist(float c1, float c2, float c3, float var)
    {
      ...
      if (idx4>=LUM_BUCKETS) idx4 = VAR_BUCKETS-1;
      //printf("%d %d\n", idx1, idx2);
...
public:
  ...
  void Reset() { background_set = target_set = 0; }

  int IsActive()
  { return target_set; }

  // add a target; by default will be searched for
  int AddPartial(YARPImageOf<YarpPixelBGR>& src, YARPImageOf<YarpPixelFloat>& mask, YARPImageOf<YarpPixelFloat>& var,int fore=1,int dx=0, int dy=0);
...
//  sb.RotateAboutCenter(theta);
  ...
  if (active)
	...
	{
	  best = AlignBlobHelper(src,ref,dest_aux,bx,by,60,10,"coarse");
...
//      boss.v_local = boss.Compare(seeker2);
  ...
  IMGFOR(dest,x,y)
    {
      ...
      //out = 255.0*3*target.GetHist(c1,c2,c3)/(target.GetHistTotal()+1.0);
...
{
  float diff = alt.target.Compare(target);
  return diff;
}
