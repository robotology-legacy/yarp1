...
public:
	YARPColorLocator (int size, int nbinsval, int nbinssat, int nbinshue, double luma_thr, double sat_thr);
	YARPColorLocator ();
	~YARPColorLocator ();

	void Resize (int size, int nbinsval, int nbinssat, int nbinshue, double luma_thr, double sat_thr);
	...
	void BackProjectWithLuma (const YARPImageOf<YarpPixelBGR>& src, YARPImageOf<YarpPixelBGR>& dest);
	...
	double CompareHistoWithLuma (CStaticLut<int>& x1, CStaticLut<int>& x2, CStaticLut1D<int>& l1, CStaticLut1D<int>& l2);

	void AddExpectancy (YARPImageOf<YarpPixelBGR>& img);
	void GetExpectancy (double& mean, double& stddev);

	int SaveHistogram (char *filename);
};
