...
int YARPSearchRotation::EstimateGoodness (YARPImageOf<YarpPixelMono>& mask, double ave, double std)
{
	...
	if (fabs(counter-ave) > std*GOODNESS)
		return -1;
	else
		return 0;
}
	...
	)
{
	...
	if (n.size() > m_threshold)
	{
		...
		while (!n.EndCursor ())
		{
			...
			if (EstimateGoodness (data.m_mask, ave, std) < 0)
			{
