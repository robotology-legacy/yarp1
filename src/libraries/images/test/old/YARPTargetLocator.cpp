...
YARPColorLocator::YARPColorLocator (int size, int nbinsv, int nbinssat, int nbinshue, double luma_thr, double sat_thr) 
	: m_histo (0.0, 255.0, 0.0, 255.0, 0.0, 255.0, nbinsv, nbinssat, nbinshue)
{
	// luma and saturation are thresholded.
	m_luma_thr = luma_thr;
	m_sat_thr = sat_thr;
...
void YARPColorLocator::Resize (int size, int nbinsv, int nbinssat, int nbinshue, double luma_thr, double sat_thr) 
{
	m_histo.Resize (0.0, 255.0, 0.0, 255.0, 0.0, 255.0, nbinsv, nbinssat, nbinshue);
	m_nbinssat = nbinssat;
	m_nbinshue = nbinshue;
	m_nbinsv = nbinsv;
...
void YARPColorLocator::CumulateHistograms (const YARPImageOf<YarpPixelHSV>& src, const YARPImageOf<YarpPixelMono>& mask, bool reset)
{
	...
	for (int i = 0; i < h; i++)
	{
		...
		for (int j = 0; j < w; j++)
		{
			...
			if (*m != 0)	// not within the mask.
			{
				if (*sat2 > m_sat_thr_int)
				{
					hue = double (*hue2);
					saturation = double (*sat2);
					value = double (*val2);

					m_histo (value, saturation, hue) ++;
					if (m_histo (value, saturation, hue) > m_histo.GetMaximum ())
						m_histo.GetMaximum () = m_histo (value, saturation, hue);
				}
				else
				{
					// no hue not saturation, use a common bin.
					hue = 0;
					saturation = 0;
					value = double (*val2);
	...
	// use to store the number of points. 
	// GetDropped() / m_numberofframes is the average # of points used.
	m_histo.GetDropped() += double(counter);
	m_histo.GetDroppedStd() += (double(counter)*double(counter));
...
double YARPColorLocator::CompareHisto (CStaticLut3D<int>& x1, CStaticLut3D<int>& x2)
{
	int *p1 = x1.GetArrayPtr()[0][0];
	int *p2 = x2.GetArrayPtr()[0][0];
	...
	const int size = x1.GetWidth() * x1.GetHeight() * x1.GetDepth();

	double mm1 = 1;
	double mm2 = m_numberofframes; //(????????????????)

	// GetDropped is the number of points used.
	//mm1 *= x1.GetDropped();
	//mm2 *= x2.GetDropped();
	
	//printf ("number of points: %lf %lf\n", x1.GetDropped(), x2.GetDropped());
...
int YARPColorLocator::ImprovedSearch (YARPImageOf<YarpPixelBGR>& img, double exx, double exy, double x, double y, double& xx, double& yy)
{
	...
	CStaticLut3D<int> comp (0.0, 255.0, 0.0, 255.0, 0.0, 255.0, m_nbinsv, m_nbinssat, m_nbinshue);
		...
		for (int ii = i; ii <= i+ey; ii++)
		{
			for (int jj = x1; jj <= x1+ex; jj++)
			{
				...
				if (sat2 > m_sat_thr_int)
				{
					...
					comp (value, saturation, hue) ++;
					if (comp (value, saturation, hue) > comp.GetMaximum ())
						comp.GetMaximum () = comp (value, saturation, hue);
				}
				else
				{
					hue = 0;
...
//		printf ("score: %lf\n", score);
		...
		for (int j = x1+1; j <= x2; j++)
		{
			...
			// research max...
			const int size = comp.GetHeight() * comp.GetWidth() * comp.GetDepth();
			int *ptr = comp.GetArrayPtr()[0][0];
			int max = 0;
...
void YARPColorLocator::BackProject (const YARPImageOf<YarpPixelBGR>& src, YARPImageOf<YarpPixelBGR>& dest)
{
	...
	for (int i = 0; i < h; i++)
	{
		for (int j = 0; j < w; j++)
		{
			...
			double value = double(*(s+2));

			*b = float(m_histo (value, sat, hue)) / float(max) * 255.0;
