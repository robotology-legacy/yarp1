...
void main ()
{
	...
	// Constants:
		...
		int step  =  20;
		int frame;
		int Mosaic_Width  = 2816;
		int Mosaic_Height = 2112;
		int RemXSize  = 1090;
		int RemYSize  = 1090;
	...
	// Set Tables
		...
		Param = Set_Param(RemXSize,RemYSize,
						  RemXSize,RemYSize,
						  152,252,42,
						  1090,
						  CUSTOM,
		...
		TabPtr.SmMosaicX = Mosaic_Width/16;
		TabPtr.SmMosaicY = Mosaic_Height/16;
		TabPtr.LgMosaicX = Mosaic_Width;
		TabPtr.LgMosaicY = Mosaic_Height;

		Init_Mosaic(&TabPtr,&ImgPtr,&Param,first);
		TabPtr.Positions = (int *)malloc((last+1)*sizeof(int));
		TabPtr.Positions [first] = (TabPtr.SmMosaicX -Param.Size_X_Remap ) * (TabPtr.SmMosaicY-Param.Size_Y_Remap) / 2 + (TabPtr.SmMosaicX-Param.Size_X_Remap)/2;
		ImgPtr.Corr_Map = (unsigned char *) malloc ((TabPtr.SmMosaicX-Param.Size_X_Remap) * (TabPtr.SmMosaicY-Param.Size_Y_Remap) * sizeof(unsigned char));


/*DEBUG
		sprintf(File_Name,"%s%03d%s","C:\\Temp\\From Talia\\Frames\\Frame_",80,".bmp");
		ImgPtr.LP_Image[1] = Read_Bitmap(&Param.Size_Theta,&Param.Size_Rho,&Param.LP_Planes,File_Name);
		ImgPtr.Small_Remapped[1] = (unsigned char *) malloc (3 * 1090 * 1090 * sizeof (unsigned char));
		Param.Size_X_Remap = 1090;
		Param.Size_Y_Remap = 1090;
		Param.Size_Img_Remap = 1090 * 1090;
		Param.Zoom_Level = 1.0;
		Remap_Cropped(ImgPtr.Small_Remapped[1],ImgPtr.LP_Image[1],&Param,TabPtr.RemapMap[0],0,0);
 		sprintf(File_Name,"%s%03d%s","C:\\Temp\\From Talia\\Frames\\Frame_",80,"Rem.bmp");
		Save_Bitmap(ImgPtr.Small_Remapped[1],1090,1090,3,File_Name);
*/

	//Load a new frame and correlate it with the Small Mosaic
		for (frame = first+step; frame<=last; frame+=step)
		{
			sprintf(File_Name,"%s%03d%s","C:\\Temp\\From Talia\\Frames\\Frame_",frame,".bmp");
			ImgPtr.LP_Image[1] = Read_Bitmap(&Param.Size_Theta,&Param.Size_Rho,&Param.LP_Planes,File_Name);

			TabPtr.Positions [frame] = Corr_w_Mosaic(&TabPtr,&ImgPtr,&Param,frame);

			CurrPosX = Param.Size_X_Remap/2 + TabPtr.Positions [frame]%(TabPtr.SmMosaicX-Param.Size_X_Remap);
			CurrPosY = Param.Size_Y_Remap/2 + TabPtr.Positions [frame]/(TabPtr.SmMosaicX-Param.Size_X_Remap);

			//Find the closest image
			MinDist = TabPtr.SmMosaicX * TabPtr.SmMosaicX + TabPtr.SmMosaicY * TabPtr.SmMosaicY;

			for (j=first; j<frame; j+=step)
			{	
				TestPosX = Param.Size_X_Remap/2 + TabPtr.Positions [j]%(TabPtr.SmMosaicX-Param.Size_X_Remap);
				TestPosY = Param.Size_Y_Remap/2 + TabPtr.Positions [j]/(TabPtr.SmMosaicX-Param.Size_X_Remap);
				CurrDist = TestPosX * TestPosX + TestPosY * TestPosY;
				if (CurrDist < MinDist)
				{
					MinDist = CurrDist;
					ClosestFrame = j;
			...
			for (j=3; j>=0; j--)
			{
				...
				MapSize = (int)(Param.Resolution * 0.125 + 0.5);
				Param.Size_X_Remap = (int)(0.5 * TabPtr.RemLUTFullX[3] * sqrt(2));
				...
				MapSize /= 4;
//				Param.Size_X_Remap = (int)(0.125 * TabPtr.RemLUTFullX[3] * sqrt(2));
				if (j==3)
					Param.Size_X_Remap = (int)(0.125 * TabPtr.RemLUTFullX[3] * sqrt(2));
				else
					Param.Size_X_Remap = Param.Size_X_Remap-8;
				Param.Size_Y_Remap = Param.Size_X_Remap;
				Param.Size_Img_Remap = Param.Size_X_Remap * Param.Size_Y_Remap;
				Size2 = Param.Size_X_Remap;

				ImgPtr.Small_Remapped[1] = (unsigned char *)malloc(Param.Size_Img_Remap*3*sizeof(unsigned char));
				Remap_Cropped(ImgPtr.Small_Remapped[1],ImgPtr.LP_Image[1],&Param,TabPtr.RemapMap[j],0,0);
				sprintf(File_Name,"%s%03d%s%2.3f%s","C:\\Temp\\From Talia\\Frames\\Frame_",frame,"Rem",Param.Zoom_Level,".bmp");
				Save_Bitmap(ImgPtr.Small_Remapped[1],Param.Size_X_Remap,Param.Size_Y_Remap,3,File_Name);

				TabPtr.Raw_Corr_Map = Find_Max_Corr(Size1,Size1,
						Size2,Size2,
						ImgPtr.Small_Remapped[0],
						ImgPtr.Small_Remapped[1]);
	...
	//				CorrelMap[3*i+2] = (unsigned char)          ((int)(255*255*255*(CorrVal[i]/MAX))%(256*256));
				...
				Save_Bitmap(ImgPtr.Corr_Map,(Size1-Size2),(Size1-Size2),1,File_Name);
...
//			Param.Center_X_Remap = (i%(TabPtr.SmMosaicX-Param.Size_X_Remap)+Param.Size_X_Remap/2);
//			Param.Center_Y_Remap = (i/(TabPtr.SmMosaicX-Param.Size_X_Remap)+Param.Size_Y_Remap/2);
			Param.Zoom_Level = 1.0/16.0;
			...
			sprintf(File_Name,"%s","C:\\Temp\\From Talia\\Test Mosaic\\Small Mosaic.bmp");
//			Save_Bitmap(ImgPtr.Small_Mosaic,TabPtr.SmMosaicX,TabPtr.SmMosaicY,3,File_Name);
//			Save_Bitmap(TabPtr.Small_Mosaic_Map,TabPtr.SmMosaicX,TabPtr.SmMosaicY,1,File_Name);

			Add_to_Full_Image(ImgPtr.Small_Remapped[1],
							  ImgPtr.Small_Mosaic,
							  TabPtr.Small_Mosaic_Map,
							  TabPtr.Small_Partial_Map,
							  TabPtr.SmMosaicX,
							  &Param);
//			Save_Bitmap(TabPtr.Small_Mosaic_Map,TabPtr.SmMosaicX,TabPtr.SmMosaicY,1,File_Name);

			free(ImgPtr.LP_Image[0]);
			free(ImgPtr.LP_Image[1]);

//			sprintf(File_Name,"%s%03d%s","C:\\Temp\\From Talia\\Test Mosaic\\Sequence\\Small Remapped",frame,".bmp");
//			Save_Bitmap(ImgPtr.Small_Remapped[1],Param.Size_X_Remap,Param.Size_Y_Remap,3,File_Name);
		
		}
		sprintf(File_Name,"%s","C:\\Temp\\From Talia\\Test Mosaic\\Small Mosaic.bmp");
		Save_Bitmap(ImgPtr.Small_Mosaic,TabPtr.SmMosaicX,TabPtr.SmMosaicY,3,File_Name);
		free(TabPtr.Positions);
		free(ImgPtr.Corr_Map);
}
/*		
