...
int find_winner_euc(struct entries_tag *codes, struct data_entry_tag *sample, 
...
int find_winner_knn(struct entries_tag *codes, struct data_entry_tag *sample, 
		    struct winner_info *win, int knn)
{
  struct data_entry_tag *codetmp;
  int dim, i, j, masked;
...
float vector_dist_euc(struct data_entry_tag *v1, struct data_entry_tag *v2, int dim)
{
  ...
  return float(sqrt(difference));
}

/* adapt_vector - move a codebook vector towards another vector */

void adapt_vector(struct data_entry_tag *codetmp, struct data_entry_tag *sample, 
		  int dim, float alpha)
{
  int i;
...
#define time_t long
  ...
  if (currt!=prevt || !rlen) {
    t1=currt-startt;
    if (rlen!=totlen) t2=(long)((currt-startt)*(float)totlen/(totlen-rlen));
    else t2=0;
    if (t2>9999) {
      t1/=60;
      t2/=60;
...
//    if ((shot->filename = malloc(strlen(filename) + 1)) == NULL)
    if ((shot->filename = new char[strlen(filename) + 1]) == NULL)
      {
...
void free_snapshot(struct snapshot_info *shot)
{
  if (shot)
    {
      if (shot->filename)
		delete[] shot->filename; // free(shot->filename);
      delete shot; // free(shot);
    }
}
...
#define INV_ALPHA_CONSTANT 100.0f
