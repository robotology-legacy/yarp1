...
static int enlarge_array()
{
  ...
  labs = new char *[lab_array_size];
  memcpy (labs, labels, sizeof(char *)*(lab_array_size - LABEL_ARRAY_SIZE));
  delete[] labels;
...
int set_entry_label(struct data_entry_tag *entry, int label)
{
...
int get_entry_labels(struct data_entry_tag *entry, int i)
{
...
void clear_entry_labels(struct data_entry_tag *entry)
{
  if (entry->num_labs > 1)
    //free(entry->lab.label_array);
	delete[] entry->lab.label_array;
...
int add_entry_label(struct data_entry_tag *entry, int label)
{
  int *atable = NULL;
  ...
  if (entry->num_labs == 1)
    {
	  ...
	  atable = new int[ATABLE_INCREMENT]; 
      if (atable == NULL)
		return ERR_NOMEM;
  ...
  if ((entry->num_labs % ATABLE_INCREMENT) == 0)
    {
      /* need more space */
		int *tmp = new int[entry->num_labs + ATABLE_INCREMENT];
		memcpy (tmp, atable, sizeof(int) * entry->num_labs);
		delete[] atable;
		atable = tmp;
...
int copy_entry_labels(struct data_entry_tag *dest, struct data_entry_tag *source)
{
  ...
  if (source->num_labs <= 1)
  ...
  else
    {
      ...
      //atable = malloc(size);
	  atable = new int[blocks * ATABLE_INCREMENT];
      if (atable == NULL)
	return ERR_NOMEM;
