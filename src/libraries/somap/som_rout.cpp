...
struct entries_tag *randinit_codes(
				...
				float* max)
{
  ...
  struct data_entry_tag *entr;
  struct data_entry_tag *maval, *mival;
  eptr p;
  long *compcnt = NULL;
...
//  }
  ...
  // max and min for each component.
  for (i = 0; i < data->dimension; i++) {
    maval->points[i] = max[i];
    mival->points[i] = min[i];
  }
...
//      fprintf(stderr, "randinit_codes: warning! component %d has no data, using 0.0\n", (int)(i + 1));
  ...
  while (entr != NULL) {
    for (i = 0; i < dim; i++) {
//      if (compcnt[i] > 0)
	entr->points[i] = (float)(mival->points[i] +
          (maval->points[i] - mival->points[i]) * ((double) orand() / 32768.0));
//      else
//	entr->points[i] = 0.0;
    }
...
struct data_entry_tag *find_eigenvectors(struct entries_tag *data)
{
...
//  float *v=(float*)malloc(2*n*sizeof(float));
  ...
  k2 = new long[n]; // malloc(n *sizeof(long));
  if (k2 == NULL)
    goto everror;
  ...
  delete[] k2; k2 = NULL; // free(k2); k2 = NULL;
...
struct entries_tag *lininit_codes(struct entries_tag *data, int topol, int neigh, 
			      int xdim, int ydim)
{
  long i, number_of_codes;
  int index, dim;
  float xf, yf;
  struct data_entry_tag *mean, *eigen1, *eigen2, *entr;
  struct entries_tag *codes;
  eptr p;
...
void bubble_adapt(struct teach_params *teach, struct data_entry_tag *sample,
		  int bx, int by, float radius, float alpha)
{
  long index;
  int tx, ty, xdim, ydim;
  struct entries_tag *codes = teach->codes;
  MAPDIST_FUNCTION *dist = teach->mapdist;
  struct data_entry_tag *codetmp;
...
void gaussian_adapt(struct teach_params *teach, struct data_entry_tag *sample,
		    int bx, int by, float radius, float alpha)
{
  long index;
  int tx, ty, xdim, ydim;
...
struct entries_tag *som_training(struct teach_params *teach)
{
  ...
  struct entries_tag *data = teach->data;
  struct entries_tag *codes = teach->codes;
...
float bubble_qerror(struct teach_params *teach, struct data_entry_tag *sample,
...
float gaussian_qerror(struct teach_params *teach, struct data_entry_tag *sample,
		     int bx, int by, float radius)
{
  long index;
  int tx, ty, xdim, ydim;
