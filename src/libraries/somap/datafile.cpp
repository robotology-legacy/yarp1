...
struct entries_tag *open_data_file(char *name)
{
  struct entries_tag *entries;
...
struct entries_tag *alloc_entries(void)
{
  struct entries_tag *en;

  //if ((en = malloc(sizeof(struct entries_tag))) == NULL)

  en = new (struct entries_tag);
  if (en == NULL)
  {
...
/* copy_entries - create a new entries structure with the same parameters
   as the original. Doesn't copy data_entry_tags */

struct entries_tag *copy_entries(struct entries_tag *entr)
{
  struct entries_tag *news;

  news = alloc_entries();
  if (news == NULL)
    return NULL;

  news->dimension = entr->dimension;
  news->topol = entr->topol;
  news->neigh = entr->neigh;
  news->xdim = entr->xdim;
  news->ydim = entr->ydim;
...
int read_headers(struct entries_tag *entries)
{
...
struct entries_tag *open_entries(char *name)
{
  struct entries_tag *entries;
...
void close_entries(struct entries_tag *entries)
{
  if (entries)
    {
      /* deallocate data */
...
struct entries_tag *read_entries(struct entries_tag *entries)
{
  long noc = 0;
  struct data_entry_tag *entr, *prev, *next;
  struct file_info *fi = entries->fi;
...
int save_entries_wcomments(struct entries_tag *codes, char *out_code_file, char *comments)
{
  struct file_info *fi;
  struct data_entry_tag *entry;
  eptr p;
  int error = 0;
...
int write_header(struct file_info *fi, struct entries_tag *codes)
{
  FILE *fp = fi2fp(fi);
...
int write_entry(struct file_info *fi, struct entries_tag *entr, 
		struct data_entry_tag *entry)
{
  FILE *fp = fi2fp(fi);
  int i, label;
...
/* initialize and possibly allocate room for data_entry_tag. If entry is NULL, 
...
struct data_entry_tag *init_entry(struct entries_tag *entr, struct data_entry_tag *entry)
{
  clear_err();
  if (entry == NULL)
  {
      //entry = malloc(sizeof(struct data_entry_tag));
	  entry = new (struct data_entry_tag);
      if (entry == NULL)
		{
		  perror("alloc_entry");
		  SOM_ERROR(ERR_NOMEM);
      ...
      entry->points = new float[entr->dimension]; // calloc(entr->dimension, sizeof(float));
	  memset (entry->points, 0, sizeof(float)*entr->dimension);
  ...
  if (entry->mask)
  {
      delete[] entry->mask; //free(entry->mask);
      entry->mask = NULL;
  }
  ...
  if (entry->fixed)
  {
      delete entry->fixed; //free(entry->fixed);
      entry->fixed = NULL;
  }
...
/* free_entry - deallocates a data_entry_tag. */

void free_entry(struct data_entry_tag *entry)
{
  if (entry)
    {
      if (entry->points)
		delete[] entry->points; // free(entry->points);
      if (entry->fixed)
		delete entry->fixed; // free(entry->fixed);
      if (entry->mask)
		delete[] entry->mask; // free(entry->mask);
...
/* load_entry - loads one data_entry_tag from file associated with entr. If 
   entry is non-NULL, an old data_entry_tag is reused, otherwise a new entry 
   is allocated. Returns NULL on error. */

struct data_entry_tag *load_entry(struct entries_tag *entr, struct data_entry_tag *entry)
{
  int i;
  float ent;
  char lab[STR_LNG];
...
struct data_entry_tag *next_entry(eptr *ptr)
{
  struct entries_tag *entries = ptr->parent;
  struct data_entry_tag *next, *current;
...
/* rewind_entries - go to the first entry in entries list. Returns pointer
   to first data_entry_tag. Loads data from file if it hasn't been loaded yet and
   rewinds file if we are using buffered reading. */

struct data_entry_tag *rewind_entries(struct entries_tag *entries, eptr *ptr)
{
  struct data_entry_tag *current = entries->entries;
  struct file_info *fi;
...
void free_entrys(struct data_entry_tag *data)
{
  struct data_entry_tag *tmp;
...
struct data_entry_tag *copy_entry(struct entries_tag *entries, struct data_entry_tag *data)
{
  int i;
  struct data_entry_tag *tmp;
  ...
  if (data->mask)
    {
	  ...
	  if ((tmp->mask = new char[entries->dimension]) == NULL)
	{
  ...
  if (data->fixed)
    {
      ...
      if ((tmp->fixed = new struct fixpoint) == NULL)
	{
	  free_entry(tmp);
	  SOM_ERROR(ERR_NOMEM);
	  return NULL;
...
struct data_entry_tag *randomize_entry_order(struct data_entry_tag *entry)
{
  long i, nol;
  struct data_entry_tag *temp, fake, newlist, *prev;
...
void set_buffer(struct entries_tag *ent, long buffer)
{
...
int set_teach_params(struct teach_params *params, struct entries_tag *codes, 
		     struct entries_tag *data, long dbuffer)
{
  int error = 0;
