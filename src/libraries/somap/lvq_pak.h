...
struct entries_tag {
  ...
  struct data_entry_tag *current;  /* current entry */
  struct data_entry_tag *entries;  /* pointer to entries */
...
typedef void VECTOR_ADAPT(struct data_entry_tag *c, struct data_entry_tag *s, 
			  int d, float a);
...
typedef float DIST_FUNCTION(struct data_entry_tag *v1, struct data_entry_tag *v2, int dim);

typedef int WINNER_FUNCTION(struct entries_tag *codes, struct data_entry_tag *sample, struct winner_info *w, int knn);
...
struct entry_ptr {
  struct data_entry_tag *current;
  struct entries_tag *parent;
  long index;
};
