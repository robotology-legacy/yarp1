...
static void cleanup_mutex (void *vm)
{
  Semaphore *s = (Semaphore *)s;
  pthread_mutex_t * m = (pthread_mutex_t *) vm;
...
semaphore_up (Semaphore * s)
{
    ...
    value_after_op = s->v;
    if (value_after_op<10000)
      {
	(s->v)++;
      }
    value_after_op = s->v;
    //printf("** boing 3 %d %ld\n", value_after_op, (long int)s);
    assert(value_after_op<=10000);
...
semaphore_down (Semaphore * s)
{
    ...
    tw_pthread_mutex_lock (&(s->mutex));
    pthread_cleanup_push(cleanup_mutex, &(s->mutex));
    while (s->v <= 0)
    {
