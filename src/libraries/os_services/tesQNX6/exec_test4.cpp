...
public:
  virtual void Body()
	...
	cout.flush();
	out.Post();
	id = YARPSocketNameService::LocateName(REG_LOCATE_NAME);
      } while (!id.IsValid());
  ...
  virtual void Body()
    {
      ...
      int result = YARPSocketNameService::RegisterName(REG_TEST_NAME);
      out.Wait();
      cout << "*** The assigned port is " << result 
	   << endl;
      cout.flush();
      out.Post();
      ...
      while (1)
	{
	  ...
	  YARPNameID id = YARPSocketSyncComm::BlockingReceive(YARPNameID(),
							imsg);
	  out.Wait();
	  ...
	  YARPSocketSyncComm::Reply(id,omsg);
