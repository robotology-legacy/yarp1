...
#define DISASTER (999999999999.0)
#define IMAGE_HEIGHT (IMG_H)
#define IMAGE_WIDTH (IMG_W)
			  ...
			  int BLOCK_SIZEY, int NORMALIZE)
{
  ...
  int i, j;
  float d, diff=0;
  if ((c1+delx>=IMAGE_WIDTH)  || (c1-delx<0) || 
      (r1+dely>=IMAGE_HEIGHT) || (r1-dely<0) ||
      (c2+delx>=IMAGE_WIDTH)  || (c2-delx<0) || 
      (r2+dely>=IMAGE_HEIGHT) || (r2-dely<0))
    {
      return DISASTER;
    }
  if (!NORMALIZE)
...
{
  int r2=0, c2=0;
  float best = DISASTER, current, central=0;
  float worst = 0;
  int i, j;
  ...
  for (i=-dely+OFFSET_Y; i<=dely+OFFSET_Y; i++)
	...
	{
	  current = ComparePixel(s1,r1,c1,s2,(*pr2)+i,(*pc2)+j,BLOCK_SIZEX,BLOCK_SIZEY,NORMALIZE);
	  if (i==0 && j==0)
	    {
	      central = current;
	    }
