...
// VIDEOBOX PATCH BEGINS MOD003
int Frame::ReturnFrame( void *rgb, int16_t **channels )
{
	AudioInfo info;

	assert(GetAudioInfo( info ) == true);
	...
	dv_encode_full_frame( encoder, pixels, e_dv_color_rgb, data );
	dv_encode_full_audio( encoder, channels, info.channels, 
			      info.frequency, data );
