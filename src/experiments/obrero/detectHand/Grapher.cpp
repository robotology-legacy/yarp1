...
class SDL_Helper {
public:
  SDL_Helper(int w, int h, const char *cap=0) {
    screen = NULL;
    pf = NULL;
    screenWidth = w;
    screenHeight = h;
...
Grapher::Grapher(int sizeX, int sizeY, const char *cap)
{
  helper=0;
  helper = new SDL_Helper(sizeX, sizeY, cap); 
}

Grapher::~Grapher()
{
  if (helper!=0)
    delete ((SDL_Helper *)helper);
}

void Grapher::Apply(YARPImageOf<YarpPixelRGB>& src) 
{
  // TODO: check image and window size
  if (helper!=0)
      ((SDL_Helper *)helper)->show(src);
}
