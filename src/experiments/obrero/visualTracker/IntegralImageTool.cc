			       ...
			       int dx, int dy, int clean)
{
  ...
  ::Offset(src,dest,dx,dy,clean,(YarpPixelFloat)0);
}
...
{
  SatisfySize(src,dest);	
  ::Offset(src,dest,dx,dy,clean,(YarpPixelMono)0);
}
...
void IntegralImageTool::MaskOffset(YARPImageOf<YarpPixelFloat>& dest, int delta)
{
  ::MaskOffset(dest,delta);
}

void IntegralImageTool::MaskOffset(YARPImageOf<YarpPixelMono>& dest, int delta)
{
  ::MaskOffsetT(dest,delta,(YarpPixelMono)0);
}
