...
static YARPImageOf<YarpPixelMono> mshifts[FINE_ORIENTATION_SHIFTS], 
		mmean;
...
static float pdist(YarpPixelBGR& pix0, YarpPixelBGR& pix1)
{
				...
				YARPImageOf<YarpPixelFloat>& mdata)
{
  int view = (dest.GetWidth()>0);
  YARPImageOf<YarpPixelFloat>& orient = fine_orientation_data.Orient();
  static YARPImageOf<YarpPixelMono> mask, rmono;
  static IntegralImageTool ii;
  rmono.CastCopy(src);
  if (view)
    {
  ...
  static YARPImageOf<YarpPixelFloat> mean, var, lx, ly, agree;
  YARPImageOf<YarpPixelMono>& mono = rmono;
  SatisfySize(mono,mean);
  SatisfySize(mono,var);
  SatisfySize(mono,lx);
  SatisfySize(mono,ly);
  ...
  IMGFOR(mono,x,y)
    {
      float total = 0;
      float total2 = 0;
      YarpPixelMono& pix0 = rmono(x,y);
      for (int k=0; k<SHIFTS; k++)
	{
	  YarpPixelMono& pix1 = mshifts[k](x,y);
