...
public:
  ...
  void TrainIgnore(YARPImageOf<YarpPixelBGR>& src,
	     YARPImageOf<YarpPixelMono>& mask,
	     YARPImageOf<YarpPixelBGR>& dest,
	     int px = -1, int py = -1);

  void WriteTrain(std::ostream& os, int all=0);
  ...
  YARPImageOf<YarpPixelFloat> salience;

  YARPImageOf<YarpPixelFloat>& GetSalience() {
	  return salience;
  }
