...
// need a hash_map implementation!
...
#include <yarp/YARPRandomNumber.h>
#include <yarp/YARPImage.h>
#include <yarp/YARPImageFile.h>
#include <yarp/YARPImageDraw.h>
#include <yarp/YARPBinaryAnalysis.h>
#include <yarp/IntegralImageTool.h>
#include <yarp/YARPShapeBoundary.h>
#include <yarp/YARPImageLabel.h>
#include <yarp/YARPFineOrientation.h>
#include <yarp/YARPNetworkTypes.h>
#include <yarp/YARPSemaphore.h>
#include <yarp/YARPVisualSearch.h>

#include <yarp/YARPSpotter.h>

#include <yarp/fit_ellipse.h>


#include <yarp/YARPImage.h>
#include <yarp/YARPImageDraw.h>

#include <yarp/graph.h>
...
YARPImageOf<YarpPixelFloat> global_salience;
...
//#define hash_map ACE_Hash_Map_Manager_Ex
#define for if (1) for
//#include <yarp/my_hash_map.h>
//#define hash_map my_hash_map
//#include <yarp/hashmap.h>
#include <stlport/hash_map>
#define hash_map stlport::hash_map
#define hash stlport::hash
/*
istream& operator>>(istream& is, __int32& target) {
	long int i;
	is >> i;
	target = i;
	return is;
...
*/
// won't yield portable files, but __int32 is awkward on windows
typedef long int lint;
typedef long int longint;
// now coming from stlport
#endif
...
static float g_found_strength = 0;
...
static void GroupOptim(YARPImageOf<YarpPixelBGR>& srcb, 
		       YARPImageOf<YarpPixelBGR>& dest, float p1, float p2)
{
  YARPImageOf<YarpPixelBGR> src;
  src.ScaledCopy(srcb,DIM,DIM);
  ...
  YARPImageOf<YarpPixelBGR> destb;
  destb.ScaledCopy(dest,srcb.GetWidth(),srcb.GetHeight());
  dest = destb;
}
...
#else
#define BASE "y:/ygarbage/"
//#define BASE "c:\\yarptest2\\"
#endif
...
//#define printf fflush(stdout); if (0) printf
...
long int isqrt(long int x) {
	//cout << "isqrt " << x << endl;
	static long int table1[1000];
	static long int table10[1000];
	static long int table100[1000];
...
template <class T>
inline double dist(T dx1, T dy1, T dx2, T dy2)
{
  //return isqrt(sql(dx1-dx2) + sql(dy1-dy2));
  //return sqrt(sq((double)dx1-dx2) + sq((double)dy1-dy2));
  return sqrt(sq((float)dx1-dx2) + sq((float)dy1-dy2));
}
		 ...
		 int x1, int y1)
{
  ...
  IMGFOR(ref,x,y)
      ...
      if (pix.r+pix.g+pix.b>0)
	{
	  ...
	  double v = sq(fabs((double)(pix.r-pix.g-pixs.r+pixs.g)))
	    +sq(fabs((double)(pix.g-pix.b-pixs.g+pixs.b)));
		 ...
		 int x1, int y1)
{
  ...
  double best = 1e10;
  double best_index = 0;
  for (float theta=0.0f; theta<M_PI/2; theta+=0.05f)
    {
  ...
  for (float theta=best_index-0.05f; theta<best_index+0.05f; theta+=0.005f)
    {
      RotateTemplate(ref,tmp,theta,x1,y1,100);
      double v = Compare(src,tmp,x0,y0,x1,y1);
      if (v<best)
...
GeometryOracle oracle_bank3[MAX_ORACLE]; //spares
...
#else
					  float del = 0;
					  if (dist(xtarget,ytarget,rx,ry)<15) {
						  del = r*rop;
					  }
#endif
				      // g is implicated
...
#ifdef APPROX_CIRCLE
		// need to patch up destf
		Smooth(destf,20);
#else
		// what the heck, smooth anyway
		Smooth(destf,5);
#endif
}

void Smooth(YARPImageOf<YarpPixelFloat>& destf,int strength=20) {
	static IntegralImageTool ii;
	ii.GetMean(destf,destf,strength);
}
...
#define MAX_CTS (40)
      ...
      if (match)
	{
	  ...
	  double shadow_topper[MAX_ORACLE];
...
#ifdef TESTING
		  global_salience.CastCopy(dest2);
#ifdef LOG_IMAGES
	  ...
	  double topb_shadow = shadow_topper[besti];
	  //double topb_shadow = btopper[besti];
	  ...
	  */
	  ...
	  if (dist(ptopx,ptopy,topx,topy)<25)
#else
	    stable = 1000;
	  if (1)
#endif
...
void YARPSpotter::TrainIgnore(YARPImageOf<YarpPixelBGR>& src,
			YARPImageOf<YarpPixelMono>& mask,
			YARPImageOf<YarpPixelBGR>& dest,
			int px, int py)
{
	printf("**** TRAINING to ignore\n");
  mutex.Wait();
  RedirectTrainer(oracle_bank3[0]);
  Process(src,mask,dest,0,px,py);
  //if (oracle_count<1) oracle_count = 1;
  //oracle_proto[0].Set(src,mask);
  printf("Comparing...\n");
...
void YARPSpotter::Reset() {
	// forget everything robot has seen
	//GeometryOracle blank;
	//oracle_bank[0] = blank;
	oracle_bank[0].Clear();
}
...
float YARPSpotter::GetStrength() 
{
	return g_found_strength;
}
