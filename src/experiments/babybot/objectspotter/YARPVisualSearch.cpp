...
#include <yarp/YARPColorSpaceTool.h>
...
//  float rad = sqrt(s_area/M_PI);
  for (int x=mymax((int)(s_x-dx),0); 
       x<=mymin((int)(s_x+dx),src.GetWidth()-1); x++)
    {
      for (int y=mymax((int)(s_y-dy),0); 
	   y<=mymin((int)(s_y+dy),src.GetHeight()-1); y++)
	{
...
float YARPLocalTarget::Compare(YARPLocalTarget& target)
{
  float result = 0;
  float ref = 0;
  float hc = mymax(1,(int)(hist_count+0.5));
  float target_hc = mymax(1,(int)(target.hist_count+0.5));
...
#define NBINS (CYL_BUCKETS*CYL2_BUCKETS*LUM_BUCKETS*VAR_BUCKETS)
  ...
  for (int i=0; i<NBINS; i++)
    {
      data1[i] = hist1[i]/hc;
      max1 = mymax(max1,data1[i]);
      data2[i] = hist2[i]/target_hc;
      max2 = mymax(max2,data2[i]);
    }
  no_info = (mymin(max1,max2)/10);
  float df;
  float chsq;
  float prob;
      ...
      for (int j=0; j<CYL2_BUCKETS; j++)
	{
	  for (int k=0; k<LUM_BUCKETS; k++)
	      ...
	      for (int l=0; l<VAR_BUCKETS; l++)
		{
		  ...
		  float w = mymin(v0,v1); //max(v0,v1)-min(v0,v1);
#elif COMPARISON_MODE == 1
		  assert(v0>=0 && v1>=0);
#define PARAM_C1_A 5 /* TWEAKED */
#define PARAM_C1_B 1 /* TWEAKED */
		  float w = log(PARAM_C1_A*mymin(v0,v1)+PARAM_C1_B);
		  ref += log(PARAM_C1_A*mymax(v0,v1)+PARAM_C1_B);
#endif
		  result += w;
		}
	    }
...

{
  ...
  IMGFOR(src,x,y)
	...
	{
	  rr = mymin(pix.r*factor,255.0f);
	  gg = mymin(pix.g*factor,255.0f);
	  bb = mymin(pix.b*factor,255.0f);
      ...
      if (m>128)
	{
	  ...
	  */
	  sat_ct += mymin(pix.r,mymin(pix.g,pix.b));
	  sat_ref++;
	}
    }
