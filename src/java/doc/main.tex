...
\begin{itemize}
    ...
    \item Depending on the carrier, some connections will not enter this
      phase or beyond.  For example, in the multicast protocol, certain
      connections are effectively identical despite having different
      targets, and only one should be broadcasting.
      
    \item So the Sender may be inhibited from entering this phase,
      depending on the carrier.
      
    \item The Sender may also skip this phase and proceed immediately to
      the next one, depending on the carrier.
      
    \item The Sender transmits 8 bytes: \packet{'Y' 'A' 10 0 0 0 'R' 'P'}.
      This identifies the length of the ``index-to-the-index'' as 10.
