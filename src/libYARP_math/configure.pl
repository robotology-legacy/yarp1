...
print "Entering configure process of YARP math support library...\n";
...
get_option_hash ("Compile_Math<-Lib_Debug", "FALSE", "Debug mode?", 1, \%options);
get_option_hash ("Compile_Math<-Lib_Release", "FALSE", "Release mode (optimization on)?", 1, \%options);
get_option_hash ("Compile_Math<-Lib_Install", "FALSE", "Install after compile?", 1, \%options);

# consistency check.
if ($options{"Compile_Math<-Lib_Debug"} ne "TRUE" && 
	$options{"Compile_Math<-Lib_Release"} ne "TRUE" && 
	$options{"Compile_Math<-Lib_Clean"} eq "TRUE")
{
	...
	$options{"Compile_Math<-Lib_Debug"} = "TRUE";
}
