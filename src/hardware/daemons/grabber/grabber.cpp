...
public:
  ...
  virtual void Update()
    {
      //HIT
      DBGPF2 cout << "HIT Acquiring..." << endl;

      if (acquired)
	{
	  grabber.RelinquishFrame(f);
	  acquired = 0;
	}
      grabber.AcquireFrame(f,1);
      acquired = 1;
...
public:
  ...
  ImageTestPattern() 
    {
      img.Resize(128,128,YARP_PIXEL_MONO);
      offset = 0;
    }
  virtual void Update()
    {
...
public:
  ...
  //YARPOutputPortOf<YARPGenericImage> port;
  ...
  virtual void Update()
      ...
      if (GetSource()!=NULL)
	{
	  ...
	  //port.Content().Resize(4,4,YARP_PIXEL_MONO);
	  DBGPF2 cout << "HIT preparing to Write" << endl;
	  port.Write();
	}
    }
};
...
ImageProcessor *current_processor = NULL;
int mesh_started = 0;
...
void AddProcessor(ImageProcessor *p)
{
  assert(p!=NULL);
  p->SetOwner(&mesh);
  p->SetSource(current_processor);
  current_processor = p;
}

void FindProcessor(const char *name)
{
  ImageProcessor *p = mesh.GetByLabel(name);
  if (p!=NULL)
    {
      current_processor = p;
    }
    ...
    {
      cerr << "Failed to locate processor " << name << endl;
      exit(1);
    }
}

void LabelProcessor(const char *name)
{
  current_processor->SetLabel(name);
}
...
void ProcessFlag(const char *flag_name, int argc, char *argv[])
{
  ...
  switch (item.index)
    ...
    case PARAM_OUTPUT:
      ...
      cout << endl;
      AddProcessor(new ImagePorter(argv[0],legacy));
      break;
    case PARAM_TAGGED:
      legacy = 0;
      break;
	...
	AddProcessor(new ImageResizer(p1,p2));
      }
      break;
    case PARAM_REFORMAT:
      {
	...
	AddProcessor(new ImageTyper(p1));
      }
      break;
    case PARAM_NAME:
      DemandArgs(flag_name,argc,1,0);
      LabelProcessor(argv[0]);
      break;
    case PARAM_INPUT:
      DemandArgs(flag_name,argc,1,0);
      FindProcessor(argv[0]);
      break;
    case PARAM_PATTERN:
      cout << "Adding test pattern object" << endl;
      AddProcessor(new ImageTestPattern());
      break;
    case PARAM_TIMER:
      cout << "Adding timer object" << endl;
      AddProcessor(new ImageTimer());
      break;
    case PARAM_SOK:
#ifdef SUPPORT_SOK
      DemandArgs(flag_name,argc,1,0);
...
int main(int argc, char *argv[])
{
  ...
  //  value["bgra"] = Framegrabber::PIXEL_BGRA;
  ...
  if (value.find(parameter["pixel format"].name)!=value.end())
  ...
  else
    {
      ...
      cout << "Supported formats are {mono, rgb, bgr}" << endl;
      exit(1);
    }
  assert (h>0 && w>0);
  ...
  grabber.SetSize(h,w);
  grabber.SetSubImage(top,left,bottom,right);
