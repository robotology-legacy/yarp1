...
public:
  ...
  virtual ErrorCode AcquireFrame(Frame& frame, int wait)
    { return NOT_IMPLEMENTED; }
  // Must return the old frame in order to get a new frame.

  virtual ErrorCode RelinquishFrame(Frame& frame) 
    { return NOT_IMPLEMENTED; }
...
public:
  ...
  virtual ErrorCode AcquireFrame(Frame& frame, bool wait=true);
