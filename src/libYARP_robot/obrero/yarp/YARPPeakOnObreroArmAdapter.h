...
#ifndef __PeakOnObreroArmAdapterh__
#define __PeakOnObreroArmAdapterh__

#include <yarp/YARPConfig.h>
#include <ace/config.h>
// #include <ace/log_msg.h>

#include <yarp/YARPPeakSerialDeviceDriver.h>
 ...
 * to control Obrero Arm. This class especially implements initialize and
 ...
 */
class YARPPeakOnObreroArmAdapter : public YARPPeakSerialDeviceDriver,
				   YARPGenericControlAdapter<YARPPeakOnObreroArmAdapter, YARPObreroArmParameters>
{
 public:
  /**
   * Default constructor.
   */
  YARPPeakOnObreroArmAdapter()
    {
      _initialized = false;
    }
  ...
  int initialize(YARPObreroArmParameters *par)
    {
      ...
      PeakOpenParameters op_par(par->_portName);
      op_par._njoints = _parameters->_nj;
      op_par._id = 0x00A4;

      if (YARPPeakSerialDeviceDriver::open ((void *)&op_par) != YARP_OK)
	{
	  ACE_OS::printf("YARPPeakSerialDeviceDriver: error opening device\n");
	  YARPPeakSerialDeviceDriver::close();
	  return YARP_FAIL;
	}
  ...
  int uninitialize()
    {
      if (YARPPeakSerialDeviceDriver::close() != 0)
	return YARP_FAIL;
