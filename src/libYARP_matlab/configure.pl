...
print "Entering configure process of YARP the Matlab interace library...\n";
...
get_option_hash ("Compile_Matlab<-Lib_Clean", "FALSE", "Clean first: i.e. rebuild libraries?", 1, \%options);
get_option_hash ("Compile_Matlab<-Lib_Debug", "FALSE", "Debug mode?", 1, \%options);
get_option_hash ("Compile_Matlab<-Lib_Release", "FALSE", "Release mode (optimization on)?", 1, \%options);
get_option_hash ("Compile_Matlab<-Lib_Install", "FALSE", "Install after compile?", 1, \%options);

# consistency check.
if ($options{"Compile_Matlab<-Lib_Debug"} ne "TRUE" && 
	$options{"Compile_Matlab<-Lib_Release"} ne "TRUE" && 
	$options{"Compile_Matlab<-Lib_Clean"} eq "TRUE")
{
	...
	$options{"Compile_Matlab<-Lib_Debug"} = "TRUE";
}
