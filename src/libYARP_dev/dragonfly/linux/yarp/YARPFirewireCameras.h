...
public:
  ...
  ~FWCameras() { shutdown_firewire(); }

  FWCameras(bool dma_on_in=true) { init_cameras(dma_on_in); }
  ...
  bool Capture(YARPImageOf<YarpPixelRGB> & im, int camera_num=0);
...
private:
  ...
  void init_cameras(bool dma_on_in=true){
    ...
    if(init_ok) {
      for(int cam_num=0; cam_num<GetNumberOfCameras(); cam_num++) {
