...
bool FWCameras::start_firewire(int camera_num)
{
  ...
  ************************************************************/
  ...
  if(!dma_on)
	...
	//FRAMERATE_30, FRAMERATE_15, FRAMERATE_7_5, FRAMERATE_3_75, FRAMERATE_1_875

		if(dc1394_dma_setup_capture(
				    handle, camera_nodes[camera_num],
				    camera_num, 
				    FORMAT_VGA_NONCOMPRESSED,
				    MODE_640x480_MONO,
				    SPEED_400,				 
...
static void RawToRGBIm(YARPImageOf<YarpPixelRGB> &image_out, char * buf, int buf_length, int buf_w, int buf_h)
{
  if((image_out.GetWidth() != buf_w) || (image_out.GetHeight() != buf_h))
    image_out.Resize(buf_w, buf_h);
...
//bool FWCameras::Capture(Image<RGBBytePixel> & imout, int camera_num)
bool FWCameras::Capture(YARPImageOf<YarpPixelRGB> & imout, int camera_num)
{
  if(!cam_num_ok(camera_num)) return(false);
  ...
  switch(capture_size)
    ...
    case _320x240:
      ...
      RawToRGBIm( imout, (char *) buf, buf_length, buf_w, buf_h);
      delete [] buf;
      break;
    case _160x120:
      buf_w=160; buf_h=120;
...
void FWCameras::capture_320x240_color_image(unsigned char * & buff_out, int camera_num )
{
  ...
  for(index=0; index<temp_size; index++) buff_out[index] = 0;
...
void FWCameras::capture_color_image(YARPImageOf<YarpPixelRGB> &grabbed_image_out, int camera_num )
{    
  if(!cam_num_ok(camera_num)) return;
  capture_a_frame(camera_num);
  ...
  for(h = 2; h < (frame_height-1); h=h+dh)
      ...
      for(w = 2; w < (frame_width-1); w=w+dw)
	{
	  ...
	  YarpPixelRGB& pix = grabbed_image_out.Pixel(w/dw,h/dh);
	  pix.r = r;
	  pix.g = g;
	  pix.b = b;
	}
